...
const actors = option('actors', 1_000_000);
const visible = option('visible', 20_000);
...
const vatQuality = stringOption('vat-quality', 'full');
const port = option('port', 4178);
...
try {
  ...
  // The benchmark owns its WebGPU device. Keep the empty Babylon harness on
  // WebGL2 so a second WebGPU device cannot distort timestamps or queue work.
  url.searchParams.set('backend', 'webgl2');
  url.searchParams.set('scenario', 'million-actor-integration');
  ...
  url.searchParams.set('vatQuality', vatQuality);
